* 슈슉언어 강좌를 시작하기 앞서, 이 언어는 강좌도 안보고 맘대로 만들어서 개 느리고, 닷넷입니다(?).
...
제 6장 "주석 처리"
이번엔 알려 줄 것이 하나 뿐이다. 구문 뒤에 "슉슉슉슉"을 쓴다면 그 코드는 무시된다. 아래는 예시 구문이다.
ex) 슉.슈슈슉.슈슉.쇼쇼쇼쇽.슉슉슉슉.



제 7장 "반복문"
이번 것은 기존 c계열 언어와 다른 방식이다. 함수처럼 사용하는데 매개변수는 없고 그저 반복이 가능할 뿐이다. 예시 구문부터 보여주겠다.
ex)
슈슉시슈슉.슈슈슈슉
    슈슈슉.슈슉슉슉.
시. 시.
...
여기까지 따라 왔다면 대략적으로 이해를 했을 것이다. "슈슉시슈슉" 으로 반복문 코드를 저장해 준다. 그 다음은 반복문 코드를 저장할 이름을 적는다.
이제 다음 줄로 이동하여 반복문에 쓰일 코드를 쓴다. (예시 구문에는 "슈슉슉슉" 을 출력하는 코드를 써 주었다. - 글쓴이)
코드를 다 적어 주었다면 "시. 시."로 끝을 맺어준다.

이제 반복을 할 코드들을 저장해 주었다. 이제는 그 코드를 불러와서 실행을 해 보자.
"슈슉시슈슉시" 로 시작하며, 다음에는 반복할 횟수이다. 이때까지 봐 왔던 것 처럼 "슉" 은 있던 변수를 불러와 그 변수의 값 만큼 반복해 주고 "슈슉" 은 '쇼' (+ 1) 와 '쇽' (- 1)
으로 반복할 횟수를 정한다.
